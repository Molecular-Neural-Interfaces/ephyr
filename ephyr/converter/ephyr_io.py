...
class EphyrIO:
    ...
    def ephyr_dir_of_experiment(experiment_folder: Path):
        ...
        return is_ephyr, ephyr_experiment_folder_path

    @staticmethod
    def is_valid_ephyr_folder(experiment_folder: Path):
    ...
    @staticmethod
    def convert_from_source_to_ephyr(
    ...
    @staticmethod
    def read_ephyr(ephyr_experiment_folder: Path,
                    extract_mode: int = 1,
                    extract_vector: Union[List[int], np.ndarray] = None,
                    each_point: int = 1) -> Tuple[Header, Tuple[Tuple[np.memmap, ...], ...]]:
        header_file = ephyr_experiment_folder / settings.HEADER_FILENAME
        try:
