...
class AddOnsDialog(QDialog):
    ...
    def _fetch_pyproject_info(self, add_on_path: str) -> Dict[str, object]:
        ...
        ephyr_entry_points = entry_points.get("ephyr.add_ons", {}) or {}
        result = {
            ...
            "entry_point_names": list(ephyr_entry_points.keys()),
        }
    ...
    def _run_pip(self, args: List[str], title: str):
        ...
        if process.stdout:
            ephyr_logger().info(process.stdout)
        if process.stderr: 
            ephyr_logger().warning(process.stderr)
        loading.close()
