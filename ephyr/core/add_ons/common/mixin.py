"""Base mixin with shared helpers for Ephyr add-ons.

Add-ons subclass ``(EphyrAddOnMixin, BaseAddOn)`` to get channel/group
...
class EphyrAddOnMixin:
    ...
    def addon_scope(self) -> str:
        return str(getattr(self, "_ephyr_module_name", type(self).__name__))
    ...
    def pipelines_path(self, add_on_data_dir: Path) -> Path:
        return Path(add_on_data_dir).parent / "ephyr" / PIPELINES_FILENAME
    ...
    def group_options(
        ...
        *,
        ...
        for idx, group in enumerate(channel_groups or []):
            ...
            key = EphyrAddOnMixin.group_stable_key(group)
    ...
    def resolve_groups_by_keys(
        ...
        *,
        ...
        options = EphyrAddOnMixin.group_options(channel_groups, non_aux_only=non_aux_only)
        if not options:
            return []
        ...
        for key in keys or []:
            ...
            if group is None:
                continue
            stable = EphyrAddOnMixin.group_stable_key(group)
            if stable in seen:
                continue
    ...
    def build_selection_valid_mask(
        ...
        *,
        ...
        from ephyr.core.add_ons.common.ignore import IgnoreEventsRule, build_valid_mask
