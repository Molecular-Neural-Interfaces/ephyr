...
def _add_module_add_on_classes(
    ...
    for attr_name in dir(module):
        ...
        if isinstance(attr, type) and issubclass(attr, BaseAddOn) and attr != BaseAddOn:
            add_on_obj = attr()
            setattr(add_on_obj, "_weegit_module_name", add_on_name)
            setattr(add_on_obj, "_weegit_entry_point_name", add_on_name)
            setattr(add_on_obj, "_weegit_distribution_name", distribution_name)
...
def load_installed_add_ons():
    ...
    runtime_add_ons: Dict[str, BaseAddOn] = {}
    for ep in entry_points().select(group="weegit.add_ons"):
        try:
            ...
            setattr(add_on_obj, "_weegit_module_name", add_on_name)
            setattr(add_on_obj, "_weegit_entry_point_name", add_on_name)
            setattr(add_on_obj, "_weegit_distribution_name", ep.dist.name)
            runtime_add_ons[add_on_name] = add_on_obj
...
def load_dev_add_ons():
    ...
    if dev_folder.exists():
        ...
        namespace_root = dev_folder / "weegit_add_ons"
        if namespace_root.exists():
            for package_dir in namespace_root.iterdir():
                ...
                module_path = f"weegit_add_ons.{package_name}"
                try:
                    _drop_stale_modules(module_path)
