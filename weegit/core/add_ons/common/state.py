...
``$SESSION/add_ons/data/weegit/<scope>.json`` so it survives app restarts and
...
class SessionParamStore:
    ...
    def from_add_on_data_dir(cls, add_on_data_dir: Path) -> "SessionParamStore":
        # run() receives ``.../add_ons/data/<module_name>``; the shared weegit
        # folder lives next to the per-module folders.
        return cls(Path(add_on_data_dir).parent / "weegit")
