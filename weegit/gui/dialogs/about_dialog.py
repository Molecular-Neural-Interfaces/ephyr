...
def _resolve_app_icon_path() -> Path | None:
    ...
    if meipass:
        candidates.append(Path(meipass) / "weegit_assets" / "weegit.png")
    repo_root = Path(__file__).resolve().parents[3]
    candidates.append(repo_root / "devtools" / "distribute" / "assets" / "weegit.png")
...
class AboutDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("About Weegit")
        self.setModal(True)
        self.resize(520, 300)
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel(f"Weegit v{version.__version__}", self)
