...
from weegit.core.add_ons.utils import load_installed_add_ons, load_dev_add_ons
from weegit import settings
from weegit.core.conversions.filters import ensure_filters_list
...
class QtWeegitSessionManagerWrapper(QObject):
    ...
    def _sync_add_on_setups_with_runtime(self):
        if not self._session_manager.current_user_session or not self._session_manager.weegit_experiment_folder:
            return
    ...
    def has_runtime_distribution(self, distribution_name: str) -> bool:
        ...
        if not distribution_name:
            return False
        return any(
            getattr(add_on, "_weegit_distribution_name", None) == distribution_name
    ...
    def runtime_add_on_distribution_name(self, module_name: str) -> Optional[str]:
        ...
        if add_on is None:
            return None
        return getattr(add_on, "_weegit_distribution_name", None)
    ...
    def runtime_add_on_names_for_distribution(self, distribution_name: str) -> List[str]:
        ...
        return sorted(
            ...
            if getattr(add_on, "_weegit_distribution_name", None) == distribution_name
        )
    ...
    def set_header_channel_units(self, channel_indexes: List[int], unit: VoltageUnitEnum) -> bool:
        ...
        if self._session_manager.weegit_experiment_folder:
            header_path = Path(self._session_manager.weegit_experiment_folder) / settings.HEADER_FILENAME
